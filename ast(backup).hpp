...
public:
  ...
  void llvm_compile_and_dump(){
    ...
    // BB->getTerminator()->print(errs(), false);
    // Verify the IR.
...
protected:
  ...
  static GlobalVariable *TheInts;
  static GlobalVariable *TheRealVars;
  static GlobalVariable *TheChars;
  static GlobalVariable *TheBools;
  static GlobalVariable *TheIntArrays;
  static GlobalVariable *TheCharArrays;
  static GlobalVariable *TheBoolArrays;
  static GlobalVariable *TheIntPointers;
...
public:
  ...
  virtual char get_char_var(){ return 0; }
...
public:
  ...
  virtual char get_char_var() override{
  	return '@';
  }
  virtual Types get_type() const{
    return TYPE_result;
...
public:
  Id(char const *v) {var = v; offset = -1; }
  virtual void printOn(std::ostream &out) const override {
  	//out << var;
  ...
  virtual Value* compile() const override {
  	std::string s = var;
    return Builder.CreateLoad(calcAddr(s, "Id"));
  }
private:
  Types type;
  char const *var;
...
public:
  ...
  virtual Value* compile() const override {
    char var = expr->get_char_var();
    std::string id(1,var);
    return Builder.CreateLoad(calcAddr(id,"Dereference", nullptr));
  }
private:
  Types consttype;
  Type_not_from_llvm *type;
...
public:
  ...
  virtual void sem() override { //sth for result types
    //SymbolEntry *lhs = st.lookup(lvalue->getstring());
    lvalue->sem();
    expr->sem();
    if (lvalue->get_type() == TYPE_result){
      if(!st.foundResult()){
...
public:
  ...
  virtual Value* compile() const override { 
  	// auto c = ReturnInst::Create(TheContext);
   //  return c;
    // llvm::ReturnInst *ret;
...
public:
  ...
  // void append_first(char *i) { id_list.insert(id_list.begin(), i); }
...
public:
  ...
  virtual void semfor() override {
    for (char std::string i : id_list->getlist()) {
      // std::string i = id;
...
public:
  ...
  virtual Value* compile() const override { 
  	//only works for writeInteger,writeChar
    std::string s = id;
    if (isLibFunc(s)){  //if it's a library proc
      ...
      //return Builder.CreateCall(F,expr_list->get_compiled_list());
    }
  	return nullptr;
  }
private:
  ...
  virtual Value* compile() const override { 
    std::string s = id;
      ...
      std::vector<Expr *> list;
        ...
        // if ((strcmp(id, "readReal")) == 0){
        //   //Value *n8 = Builder.CreateZExt(n, i8, "ext");
        //   return Builder.CreateCall(TheReadReal, std::vector<Value *> {  });
        // }
        ...
        if ((strcmp(id, "sqrt")) == 0){
          if (expr_list){
            list = expr_list->getlist();
            Value *n = list.front()->compile();
            //Value *ndouble = Builder.CreateZExt(n, DoubleTyID, "ext");
...
public:
  ...
  virtual Value* compile() const override { 
  	...
  	// return ConstantDataArray::getString(TheContext, name);
  	//return PointerType::get(i8, cons);
  	//return nullptr;
...
public:
  Label(char *i, Id_list *idlist){
    //id = i;
    //id = new Id(i);
    id_list = idlist;
    if (i) idlist->append_first(i);
  }
  ~Label(){
    delete id_list;
  }
...
public:
  // Decl(char *i, Id_list *idlist, Type_not_from_llvm *t){
  ...
  virtual Value* compile() const override {   
    ...
    for (char *id : id_list->getlist()){  // this is for everything not array, pointer
...
public:
  ...
  virtual void printOn(std::ostream &out) const override {
    ...
    out << "\n)";
    //out << "Problem is Local_list\n";
    //if (local_list.empty()) {out << "local_list is empty!\n";}
  }
  virtual std::string getstring() override {
    std::string s = "";
    s += "Local_list(";
